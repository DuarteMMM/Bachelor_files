...
vector<ODEPoint> ODESolver::LeapFrogSolve(double step, double time) {

  int n_functions = functions.size();
  double init_time = initial_values.getIndepVar();
  int n_steps = (time - init_time)/step;
  vector<double> init_values = initial_values.getDepVarVector();
  init_values.emplace(init_values.begin(), init_time);
  
  vector<ODEPoint> result;
  result.push_back(initial_values);

  vector<double> first; 
  first.push_back(init_time + step);
  ...
  for(int j = 0; j < n_functions; j++) {
    ...
    /*cout << prev[j] << endl;
      cout << step << endl;
      cout << functions[j].EvalPar(&(prev[0])) << endl;*/
    first.push_back(init_values[j+1] + step * functions[j].EvalPar(&(init_values[0])));

  }

  first.erase(first.begin());
    
  ODEPoint firstpoint (step + init_time, first);
  ...
  for(int i = 2; i <= n_steps; i++) {
    ...
    vector<double> prevminus1 = result[i-2].getDepVarVector();
    prevminus1.emplace(prevminus1.begin(), result[i-2].getIndepVar());
    ...
    for(int j = 0; j < n_functions; j++) {
      ...
      cout << step << endl;
      cout << functions[j].EvalPar(&(prev[0])) << endl;*/
      vec.push_back(prevminus1[j+1] + 2*step * functions[j].EvalPar(&(prev[0])));
