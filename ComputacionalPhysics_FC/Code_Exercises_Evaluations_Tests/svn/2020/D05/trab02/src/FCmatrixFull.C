...
double FCmatrixFull::GetDeterminant() const
{
  ...
  // Matriz maior (regra dos cofatores inicialmente)
	...
	const int sub_number = N - 1;
	const int sub_size = sub_number * sub_number;

	int posicao = 0;

	double *sub_array = new double[sub_size];
	...
	FCmatrixFull sub_matriz(sub_array, sub_number, sub_number);
	determinante += (1 - ((i & 1) << 1)) * M[0][i] * sub_matriz.Determinant();
	}
...
// Devolve o índice da linha que para a qual o quociente entre o valor na coluna j desse linha e o valor máximo dessa linha seja máximo (tudo em módulo)
int FCmatrixFull::GetColMax(int i, int j) const
{
  ...
  if (j < 0 || j >= m || i < 0 || i >= n)
    ...
    {
      int CurrentMax = i; // Índice da linha (a partir da linha i) com o maior valor comparativamente à soma dos elementos da sua linha (em módulo)
      ...
      for (int k = i; k < n; k++)
	{
	  ...
	  /*cout << "j: " << j << endl;
	  cout << "k: " << k << ", índice max linha: " << indice_max_linha << endl;
	  cout << "M[k][index]: " << M[k][indice_max_linha] << endl;
	  cout << "M[k][j]: " << M[k][j] << endl;*/
	  ...
	  if (CurrentMaxValue == 1.)
	    //return CurrentMax; // Pode-se já terminar o ciclo
	    break;
