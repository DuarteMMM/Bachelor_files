  ...
  for(int i = 0; i < n; i++)
    {
      ...
      //cout << "FirstDerivative(xmin + (i+1)*h): " << d.FirstDerivative(xmin + (i+1)*h) << ", d.FirstDerivative(xmin + i*h): " << d.FirstDerivative(xmin + i*h) << endl;
      //cout << "Eval(xmin + i*h): " << Eval(xmin + i*h) << ", Eval(xmin + (i+1)*h): " << Eval(xmin + (i+1)*h) << ", integral: " << integral << endl;
...
void Integrator::TrapezoidalAdaptative (int max, double& integral, double& error) {
  ...
  error = valor-integral; // Esta diferença dá uma boa estimativa do erro
}
...
void Integrator::Romberg (int& steps, int& level, double& integral, double& error) {
  ...
  if (level>steps)
	...
	for (contador_level=1; contador_level <= contador_steps; contador_level++)
	  {
	    ...
	    //cout << fixed << setprecision(20) << "Integral: " << result[contador_steps][contador_level] << ", Error: " << calc_error << endl;
	    //cout << result[contador_steps][contador_level] << endl;
	  }
	...
	//if (contador_steps > 0)
	// calc_error = (integral - result[contador_steps][contador_level-1]);

	//calc_error = pow(((xmax-xmin)/nsteps_trapezoidal),contador_steps);
	calc_error = abs((result[contador_steps][0] + var_erro - integral)/integral); // Pois I = I(h1) + E(h1) = I(h2) + E(h2), e error=E(h2)
	//cout << fixed << setprecision(20) << "Integral: " << integral << ", Error: " << calc_error << endl;
...
Integrator Integrator::operator-() {
  //notice xmax and xmin are swapped when compared to the regular constructor
