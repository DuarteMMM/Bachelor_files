...
void Derivator::FirstDer_3PointFormula (vector<double> x, vector<double> y, int index, double& value) {

  if (x.size() != y.size())
    throw invalid_argument(TString::Format("[%s] received two vectors with different lenghts!\n", __PRETTY_FUNCTION__));

  if (index > x.size()-2 || index < 1)
    throw invalid_argument(TString::Format("[%s] received an index out of range to use the three-point formula!\n", __PRETTY_FUNCTION__));

  double h_i_back = x[index]-x[index-1];
  double h_i_forward = x[index+1]-x[index];

  // Nota: a expressão no slide está mal, pois usa h_i em vez de h_i+1
 
  value = ( h_i_back*h_i_back*y[index+1] + (h_i_forward*h_i_forward - h_i_back*h_i_back)*y[index] - h_i_forward*h_i_forward*y[index-1] ) / (h_i_forward*h_i_back*(h_i_back+h_i_forward) );
